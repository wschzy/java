...
import cn.edu.store.service.UserService;
...
public class UserController extends BaseController {
	...
	@RequestMapping(method = RequestMethod.POST, value = "/checkLoginUsername.do")
	@ResponseBody
	...
	public ResponseResult<Void> updateUserInfo(String username, String phone, String email, HttpSession session) {
		...
		userService.updateUserInfo(id, username, phone, email);
	...
	@ResponseBody
	public ResponseResult<Void> handleUpdatePassword(@RequestParam("old_password") String oldPassword,
		...
		// 执行
		state = userService.updatePassword(uid, oldPassword, newPassword);
		message = state == 1 ? "修改密码成功" : "修改密码失败，旧密码不正确";
		// 准备返回值
		rr.setState(state);
	...
	/**
	 * 下载Excel application/vnd.openxmlformats-officedocument.spreadsheetml.sheet
	 */
	...
	/**
	 * 下载图片 Content-Disposition: attachment; filename="fname.ext"
	 * 
	...
	/**
	 * 生成验证码图片控制器 其中 value="code.do"用于映射URL produces="image/png" 用于设置响应头中的
	 * Content-Type 属性 返回值 byte[] 会被注解@ResponseBody自动 处理放置到响应消息的Body中发送到客户端
	 * 
	 * @ResponseBody 还会根据 byte[] 数组长度自动 设置 响应头部的Content-Length属性
	 * 
	 * @return
	 */
	...
	@ResponseBody
	public ResponseResult<Void> uploadImages(@RequestParam("images") MultipartFile[] images, HttpServletRequest request)
			throws IOException {
		// 保存到文件系统
		String path = "/images/upload";// WEB路径
		path = request.getServletContext().getRealPath(path);
