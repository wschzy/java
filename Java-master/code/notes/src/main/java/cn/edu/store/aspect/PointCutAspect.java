...
public class PointCutAspect {
	...
	/**
	 * Bean组件切入点，对两个Bean组件进行切入 如下程序将在 userService 和 dictService 全部方法之前执行 test() 方法
	 */
	...
	/**
	 * 切入点表达式 bean(*Service) 将会拦截全部的 业务层方法，每个业务层方法之前都会执行 test1()
	 * 方法。使用这种方式进行切入时候，建议有统一的 命名规范。
	 */
	// @Before("bean(*Service)")
	public void test1() {
	...
	/**
	 * 类的切入点：如下切入点表达式将test2()方法切入到 UserServiceImpl 的全部方法之前执行。
	 */
